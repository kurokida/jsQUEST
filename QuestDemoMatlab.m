...
for m=1:nSimulation
    ...
    create_time = 1000 * (eval(getSecsFunction) - baseTime);
    ...
    for k=1:trialsDesired
        ...
        fprintf(Fid, '%d,%d,%f,%d,%f,%f\n', m, k, tTest, response, QuestMean(q), QuestSd(q));
    end
    fprintf(Fid2, '%d,%f,%f,%f,%f\n', m, create_time, QuestMean(q), QuestSd(q), total_time/trialsDesired);
...
%aa = eval(getSecsFunction)-timeZero
